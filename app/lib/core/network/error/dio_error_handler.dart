    ...
    case DioExceptionType.badResponse:
      {
        if (error.response?.data is String) {
        ...
        } else if (error.response?.data['code'] != null && (error.response?.data['code'] ?? "0") != "0") {
          ...
          if (error.response?.statusCode == 200 && ("${(error.response?.data["statusCode"] ?? "0")}" != "0")) {
            ...
            final faultString = error.response?.data?['fault']?['faultstring'] ?? "Unknown Error";
            final minutesLeft = error.response?.data?["data"]?["mins_to_join"];
            errorDescription = minutesLeft != null ? "$faultString,\n Minutes left to join: $minutesLeft" : faultString;
